  ...
  TableScan: dummy [time:Timestamp(ms), value:Float64;N]"#;
    ...
    async fn mixed_native_histogram_ranges_use_coordinated_udfs() {
        ...
        for histogram_on_left in [false, true] {
            ...
            planner.ctx.start = 3000;
            planner.ctx.end = 3000;
            planner.ctx.interval = 1000;
            planner.ctx.range = Some(3000);
            ...
            assert!(
                PromPlanner::alternative_sample_range_columns(
                    input.schema(),
                    &planner.ctx.field_columns
                )
                .is_some()
            );
            let PromExpr::Call(call) = parser::parse("rate(lhs[5m])").unwrap() else {
                unreachable!()
            };
            let (exprs, _) = planner
